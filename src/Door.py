...
class Door:
    ...
    def processPygameUserEvent(self):
        ...
        if pygame.event.peek(pygame.USEREVENT):
            ...
            position = glu.gluUnProject(winX, winY, winZ, modelViewMatrix, gl.glGetDoublev(gl.GL_PROJECTION_MATRIX),
                                        gl.glGetIntegerv(gl.GL_VIEWPORT))

            # Checks if there is a section corresponding to the event position
            sectionFound = False
            for x in self.objects:
                if x.getParameter('position')[0] < position[0] < x.getParameter('position')[0] + x.getParameter(
                        'width') and \
                        x.getParameter('position')[2] < position[2] < x.getParameter('position')[2] + x.getParameter(
                    'height') and \
